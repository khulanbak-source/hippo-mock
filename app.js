/* Hippo Mock Exam — Little Hippo · Continental Round
   Content + grading + score history are server-gated (no answers in the browser).
   Flow: login (-> token) -> ready -> exam (timed) -> /api/grade -> result -> My scores. */
(function () {
  "use strict";

  var TIME_LIMIT = 2400, PASS = 75;
  var state = {
    name: "", token: "", config: {}, exams: [],        // exams = [{id,theme,batch}]
    scores: [], bestByExam: {}, passedSet: {},
    exam: null, timeLeft: TIME_LIMIT, timer: null, submitted: false,
    cellInputs: [], acrossAt: [], downAt: [], active: null, _review: null, _next: null
  };

  function $(id) { return document.getElementById(id); }
  function show(screenId) {
    ["screen-login", "screen-ready", "screen-scores", "screen-exam", "screen-result"].forEach(function (s) {
  ...
  // ---- score helpers ----
  function passedCount() { return Object.keys(state.passedSet).length; }
  function allPassed() { return state.exams.length > 0 && state.exams.every(function (e) { return state.passedSet[e.id]; }); }
  function computeDerived() {
    var m = {};
    state.scores.forEach(function (r) {
      if (r.exam == null) return;
      var e = m[r.exam] || (m[r.exam] = { best: null, attempts: 0, last: null });
      e.attempts++;
      if (r.overall != null && (e.best == null || r.overall > e.best)) e.best = r.overall;
      if (e.last == null && r.overall != null) e.last = r.overall;   // results come newest-first
    });
    state.bestByExam = m;
    state.passedSet = {};
    Object.keys(m).forEach(function (id) { if (m[id].best != null && m[id].best >= PASS) state.passedSet[id] = true; });
  }
  function refreshScores(cb) {
    api("/api/scores", { token: state.token }).then(function (s) {
      state.scores = (s && s.results) || []; computeDerived(); if (cb) cb();
    });
  }
  ...
  $("btn-scores-back").addEventListener("click", function () { updateProgressLine(); show("screen-ready"); });
  function openScores() { refreshScores(renderScores); }
  function renderScores() {
    var total = state.exams.length, n = passedCount();
    $("scores-summary").textContent = "⭐ Passed: " + n + " / " + total + " exams";
    var gate = $("scores-gate");
    if (allPassed()) {
      gate.classList.remove("hidden");
      gate.innerHTML = "🎉 <b>You passed all " + total + " exams!</b><br>To unlock the next 10, ask the admin.";
    } else { gate.classList.add("hidden"); gate.innerHTML = ""; }
    var ol = $("scores-list"); ol.innerHTML = "";
    state.exams.forEach(function (e) {
      var info = state.bestByExam[e.id], best = info ? info.best : null, attempts = info ? info.attempts : 0;
      var passed = best != null && best >= PASS;
      var badge = passed ? '<span class="sbadge pass">Passed</span>'
    ...
    ol.querySelectorAll(".retake").forEach(function (b) {
      b.addEventListener("click", function () { startExamById(parseInt(b.dataset.id, 10)); });
    });
    show("screen-scores");
  }
  ...
  $("btn-again").addEventListener("click", function () { if (state._next) state._next(); });

  function pickExamId() {
    var pool = state.exams.filter(function (e) { return !state.passedSet[e.id]; });
    if (pool.length === 0) return null;
    return pool[Math.floor(Math.random() * pool.length)].id;
  }
  function startExam() {
    var id = pickExamId();
    if (id == null) { openScores(); return; }    // all passed -> show scores + upsell
    startExamById(id);
  }
  ...
  function renderCrossword(cw) {
    ...
    var grid = $("cw-grid"); grid.innerHTML = ""; grid.style.gridTemplateColumns = "repeat(" + cw.cols + ", auto)";
    ...
    for (var r = 0; r < cw.rows; r++) {
      ...
      for (var c = 0; c < cw.cols; c++) {
        var cell = cw.cells[r][c], div = document.createElement("div");
        ...
        inp.setAttribute("autocapitalize", "characters"); inp.autocomplete = "off"; inp.setAttribute("autocorrect", "off"); inp.spellcheck = false;
        inp.dataset.r = r; inp.dataset.c = c;
        inp.addEventListener("focus", onCellFocus); inp.addEventListener("input", onCellInput);
        inp.addEventListener("keydown", onCellKey); inp.addEventListener("click", onCellClick);
        div.appendChild(inp); grid.appendChild(div); state.cellInputs[r][c] = inp;
      }
    }
    $("cw-across").innerHTML = ""; $("cw-down").innerHTML = "";
    buildEntries(cw, "across", state.acrossAt, $("cw-across")); buildEntries(cw, "down", state.downAt, $("cw-down"));
    state.active = null;
  }
  function buildEntries(cw, dir, map, listEl) {
    (cw[dir] || []).forEach(function (entry) {
      var li = document.createElement("li"); li.textContent = entry.num + ". " + entry.clue;
      li.addEventListener("click", function () { activateEntry(entry, dir, true); });
      listEl.appendChild(li); entry._li = li;
      var dr = dir === "across" ? 0 : 1, dc = dir === "across" ? 1 : 0; entry.path = [];
      for (var i = 0; i < entry.len; i++) { var rr = entry.row + dr * i, cc = entry.col + dc * i; map[rr][cc] = entry; entry.path.push({ r: rr, c: cc }); }
    });
  }
  ...
  function activateEntry(entry, dir, focusFirst) {
    state.active = { entry: entry, dir: dir }; clearHighlights();
    ...
    if (focusFirst) { var target = entry.path.find(function (p) { return !state.cellInputs[p.r][p.c].value; }) || entry.path[0]; var inp = state.cellInputs[target.r][target.c]; inp.focus(); inp.parentElement.classList.add("active"); }
  }
  function onCellFocus(e) {
    ...
    if (!state.active || (state.acrossAt[r][c] !== state.active.entry && state.downAt[r][c] !== state.active.entry)) {
      var entry = state.acrossAt[r][c] || state.downAt[r][c]; if (entry) activateEntry(entry, state.acrossAt[r][c] ? "across" : "down", false);
    }
  ...
  function onCellClick(e) {
    ...
    else if (a) activateEntry(a, "across", false); else if (d) activateEntry(d, "down", false);
  }
  function onCellInput(e) { var v = e.target.value.toUpperCase().replace(/[^A-Z]/g, ""); e.target.value = v.slice(-1); if (e.target.value) moveNext(+e.target.dataset.r, +e.target.dataset.c, 1); }
  ...
  function moveNext(r, c, sign, erase) {
    if (!state.active) return; var path = state.active.entry.path;
  ...
  function renderUoe(items) {
    ...
    items.forEach(function (item, i) {
      ...
      var inp = document.createElement("input"); inp.type = "text"; inp.className = "uoe-input"; inp.dataset.i = i;
      inp.autocomplete = "off"; inp.setAttribute("autocorrect", "off"); inp.spellcheck = false; inp.setAttribute("autocapitalize", "off"); inp.setAttribute("aria-label", "answer " + (i + 1));
  ...
  function collectUoe() { return [].map.call(document.querySelectorAll(".uoe-input"), function (i) { return i.value; }); }
  function submitExam(auto) {
    if (state.submitted) return; state.submitted = true; if (state.timer) clearInterval(state.timer);
    api("/api/grade", { token: state.token, id: state.exam.id, cw: collectCrossword(), uoe: collectUoe() }).then(function (res) {
      if (!res || !res.ok) {
      ...
      applyReview(res.review); state._review = res.review;
      refreshScores(function () { showResult(res, auto); });
    });
  }
  function applyReview(review) {
    (review.cw || []).forEach(function (d) {
      var list = state.exam.crossword[d.dir] || [], entry = list.filter(function (e) { return e.num === d.num; })[0];
  ...
  function showResult(res, auto) {
    $("score-pct").textContent = res.overall; $("score-cw").textContent = res.cwPct + "%";
    $("score-uoe").textContent = res.uoePct + "%"; $("score-overall").textContent = res.overall + "%";
    var card = document.querySelector(".result-card"); card.classList.toggle("result-fail", !res.passed);
    ...
    // primary action: go to My scores once everything is passed, else next/again
    if (allPassed()) { $("btn-again").textContent = "See my scores 📊"; state._next = openScores; }
    else { $("btn-again").textContent = res.passed ? "Next exam ▶" : "Try again ▶"; state._next = startExam; }
    show("screen-result");
  }
  $("btn-see-result").addEventListener("click", function () { $("pass-gate").classList.add("hidden"); $("score-detail").classList.remove("hidden"); });
  $("btn-result-scores").addEventListener("click", openScores);
  $("btn-review").addEventListener("click", function () {
    ...
    state._review.cw.forEach(function (d) {
      html += '<div class="review-item"><span class="' + (d.ok ? "r-ok" : "r-no") + '">' + (d.ok ? "✓" : "✗") + "</span> <b>" + d.num + " " + d.dir + ".</b> " + esc(d.clue) + ' → <span class="rt">' + esc(d.answer) + "</span>" + (d.ok ? "" : " (you wrote: " + esc(d.got || "—") + ")") + "</div>";
    });
    html += "<h3>Use of English</h3>";
    state._review.uoe.forEach(function (d, i) {
      html += '<div class="review-item"><span class="' + (d.ok ? "r-ok" : "r-no") + '">' + (d.ok ? "✓" : "✗") + "</span> <b>" + (i + 1) + ".</b> " + esc(d.q.replace("____", "[ " + d.answer + " ]")) + (d.ok ? "" : " — you wrote: " + esc(d.got || "—")) + "</div>";
    });
